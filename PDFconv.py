...
def convert_page_to_images(pdf_path, page_num, image_folder):
    ...
    for image_num, image in enumerate(images):
        image_path = os.path.join(
            image_folder, f"page_{page_num}_image_{image_num}.png")
...
def pdf_to_word(pdf_path, output_path, max_image_width=400):  # Adjust max_image_width as needed
    ...
    with ThreadPoolExecutor() as executor:
        text_futures = [executor.submit(
            extract_text_from_page, page) for page in doc]
        image_futures = [executor.submit(
            convert_page_to_images, pdf_path, i, image_folder) for i in range(len(doc))]
...
# Specify the paths
pdf_path = "Private project details (100 images).pdf"
output_path = "Private project details (100 images).docx"
